...
function! s:MultipleSearchInit()
	...
	let fgColor = s:Strntok(s:TextColorSequence, ',', loopCount + 1)
        execute 'highlight MultipleSearch' . loopCount
           \ . ' ctermbg=red   guibg=' . bgColor
           \ . ' ctermfg=white guifg=' . fgColor
...
nnoremap <silent> mn :call <SID>SearchNext(0)<CR>
"
" Set the current search pattern to the previous one in the list
nnoremap <silent> mN :call <SID>SearchNext(1)<CR>

" Clear the search
nnoremap <silent> mc :call <SID>MultipleSearchReset(1)<CR>:nohlsearch<CR>
