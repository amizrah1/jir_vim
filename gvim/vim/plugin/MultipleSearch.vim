...
if v:version < 700
    ...
    "vnoremap <silent> mm y:execute ':Search \V<C-R>=substitute(escape(@@,"/\\"),"\n","\\\\n","ge")<CR>'<CR>
    vnoremap <silent> mm y:call MultipleSearch(0,'\V'.substitute(escape(@@,"\\/\"'"),"\n",'\\n','ge'))<CR>
else
    ...
    " Following Manuel's idea, adapt the former 'Super Star' tip from vim.org to work with
    " :Search on a visual selection.
    vnoremap <silent> mm y:call MultipleSearch#MultipleSearch(0,'\V'.substitute(escape(@@,"\\/\"'"),"\n",'\\n','ge'))<CR>
endif

" Thanks to Manuel Picaza for the following mapping to :Search the word under
" the cursor.
nnoremap <silent> mm :execute ':Search \<' . expand('<cword>') . '\>'<cr>
