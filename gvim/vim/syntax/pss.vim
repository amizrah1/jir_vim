...
" Read the SV syntax to start with
if version < 600
  so <sfile>:p:h/verilog_systemverilog_uvm.vim
else
  runtime! syntax/verilog_systemverilog_uvm.vim
...
syn keyword slnStatement empty channel memory child definition body header declaration macro abstract run_start run_end file platform share function get_all_actions add_action get_type_name schedule_before
syn keyword slnStatement c_file_name const end enabled processor conditional exclusive static shared disable pool resource buffer state stream output input
syn keyword slnSpecial assign_processor_logic assign_processor_logic_no_lock

syn keyword slnStatement do exec rand rand_attr lock
syn keyword slnStatement import function void target solve
...
syn keyword slnString true false

syn keyword slnMethod allocate post_allocate pre_gen_code post_gen_code pre_gen_test_table_instance post_gen_test_table_instance post_solve post_expansion exec pre_exec on_generated pre_sample sln_thread 
syn keyword slnMethod print operator action abs_scenario abs_action rf_manager generated ended token table compound activity sln construct_nested embed_nested final post_create solve pre_solve method permute  sln_executable
...
syn keyword slnMethod extend top_action package pss out
syn keyword slnMethod      chandle coverpoint coverspec cross foreach
syn keyword slnStatement   buffer bins  enum  dynamic package sequecne void with while target if else  super override in int bit bool string enum
...
syn keyword slnStructure   component class
