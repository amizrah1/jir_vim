...
function s:GetSVBlockStart(keyword, curr_lnum, mode)
  ...
  elseif a:keyword =~ '`endif'
    ...
    let pstart = '`if\%[n]def'
    let pmid   = '`else'
  elseif a:keyword =~ '`.vm_object_utils_end\>'
    let pend   = '`.vm_object_utils_end\>'
    let pstart = '`.vm_object\%(_param\)\=_utils_begin\>'
  elseif a:keyword =~ '`.vm_component_utils_end\>'
    let pend   = '`.vm_component_utils_end\>'
    let pstart = '`.vm_component\%(_param\)\=_utils_begin\>'
  else
    let pend = '\<' . a:keyword . '\>'
...
function GetSystemVerilogIndent()
  ...
  let sv_end_match = '\<\%(' .
        ...
        \ '[})]\|' .
        \ '`\<\%(else\|endif\)\>\|' .
        \ '`\<\%(.vm_\%(object\|component\)_utils_end\)\>'
  ...
  " Indent after a '`ovm_*_utils_begin'
  elseif last_line =~ '`ovm_\%(object\|component\)_utils_begin\>' &&
    \ ( last_line2 !~ sv_openstat . '\s*$' ||
    \   last_line2 =~ '^\s*[^=!]\+\s*:\s*$' )
    let ind = ind + offset
    if vverb | echo vverb_str "Indent after ovm_utils_begin statement.\n" | endif

  " Indent after a '`.vm_*_utils_begin'
  elseif last_line =~ '`.vm_\%(object\|component\)_utils_begin\>' &&
    \ ( last_line2 !~ sv_openstat . '\s*$' ||
    \   last_line2 =~ '^\s*[^=!]\+\s*:\s*$' )
    let ind = ind + offset
    if vverb | echo vverb_str "Indent after .vm_utils_begin statement.\n" | endif
  ...
  " De-indent after `o/uvm_* macro
  if last_line =~ '^\s*`\<.vm_fatal\>' &&
   \ last_line2 =~ '^\s*\<if\>' &&
   \ curr_line !~ '^\s*\%(' . sv_end_match . '\)'
      let ind = ind - offset
      if vverb | echo vverb_str "De-indent after o/.vm macro.\n" | endif
    endif
  endif
